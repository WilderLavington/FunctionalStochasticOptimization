...
class OnlineLearningAlgo():
    ...
    def log(self):
        ...
        all_keys = deepcopy(list(info.keys()))
        for key in all_keys:
...
class FTL(OnlineLearningAlgo):
    ...
    def gd_step(self, tensor_states, tensor_expert_actions):
        ...
        def closure(verbose=False, call_backward=False):
            ...
            # grab ftl loss
            loss = self.compute_loss(tensor_states, tensor_expert_actions) / self.replay_size
    ...
    def sgd_step(self):
        ...
        for states_batch, expert_actions_batch in self.data_generator:
            ...
            loss, grad_norm = self.gd_step(states_batch, expert_actions_batch)
        # return information
    ...
    def iterative_step(self, tensor_states, tensor_expert_actions):
        ...
        if self.args.use_sgd:
            loss, grad_norm = self.sgd_step()
        else:
            loss, grad_norm = self.gd_step(tensor_states, tensor_expert_actions)
        # return it
        return loss.detach(), grad_norm.detach()
...
class FTRL(OnlineLearningAlgo):
    ...
    def step(self):
        ...
        def closure(verbose=False, call_backward=False):
            ...
            # compute
            loss = (lin_loss + scaling * tr_loss) + cum_ftl_loss
...
class AFTRL(FTRL):
        ...
        def step(self):
            ...
            def closure(verbose=False, call_backward=False):
                ...
                # pick ftrl_variant for scaling
                scaling = self.outer_lr * self.squared_gradsum.pow(0.5).detach()
        ...
        def exact_step(self, new_tensor_states, new_tensor_expert_actions):
            ...
            if self.episode > 1:
                w_k = torch.cat([self.policy.mean_linear.weight.data, self.policy.mean_linear.bias.data.unsqueeze(1)],dim=1).detach().t()
                X, b = self.policy.transform_state(new_tensor_states), new_tensor_expert_actions
                X = torch.cat((X, torch.ones(X.size()[0], 1)), dim=1)
                grad = torch.mm(X.t(), torch.mm(X, w_k) - b)
                self.squared_gradsum += grad.pow(2)
                scaling = self.outer_lr * self.squared_gradsum.pow(0.5).sum()
            else:
                w_k = torch.cat([self.policy.mean_linear.weight.data, self.policy.mean_linear.bias.data.unsqueeze(1)],dim=1).detach().t()
                w_k = 0. * w_k # assume initial parameters are zero
                X, b = self.policy.transform_state(new_tensor_states), new_tensor_expert_actions
                X = torch.cat((X, torch.ones(X.size()[0], 1)), dim=1)
                grad = torch.mm(X.t(), torch.mm(X, w_k) - b)
                self.squared_gradsum = grad.pow(2)
...
class RFTRL(FTRL):
        ...
        def __init__(self, env, args):
            super(RFTRL,self).__init__(env, args)
        ...
        def step(self):
            ...
            def closure(verbose=False, call_backward=False):
                ...
                # pick ftrl_variant for scaling
                scaling = self.outer_lr # * self.episode
