...
baselines = ['AdaOGD', 'AdamOGD', 'OSLS']
algorithms = baselines + ['SSO-'+str(m_) for m_ in m]
environment_names= ['BreakoutNoFrameskip-v4', 'PongNoFrameskip-v4']
sso_algos = ['SSO_OGD', 'SSO_SLS', 'SSO_AdaOGD']
label_map = {'log_policy_loss':'Log-Policy-Loss',
             'policy_return':'Policy-Return',
             'total_examples': 'Total Environment Interactions'}
name_mask = {'LSOpt':'SLS', 'SGD': 'SGD', 'Adam': 'Adam', 'Adagrad':'Adagrad',
             'SLS_FMDOpt': 'SSO-SLS'}

def generate_atari_figure(wandb_records, env_name='PongNoFrameskip-v2',
            fig_name='ex', include_leg=True):
